...
class Plugin : public ContextUser
{
public:
...
class Implementation : public Plugin<Implementation, int>
{
public:
...
{
public:
    std::unordered_map<CStringW, std::function<std::shared_ptr<ContextUser>(Context *, void *)>> classMap;

    Context(void)
    {
        classMap[L"imp"] = Implementation::create;
    }

    auto getCreator(LPCWSTR name)
    {
        return classMap[name];
    }

    template <typename... ARGUMENTS>
    std::shared_ptr<ContextUser> create(LPCWSTR name, ARGUMENTS... arguments)
    {
        std::tuple<ARGUMENTS...> tuple(arguments...);
        return getCreator(name)(this, reinterpret_cast<void *>(&tuple));
    }
};
...
{
    Context context;
    auto instance = context.create(L"imp", 10);
