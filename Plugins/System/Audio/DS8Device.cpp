...
*/
		...
		public:
				...
				: ContextRegistration(context)
			{
				...
				{
					throw std::runtime_error("Unable to create sound controller");
				}

				resultValue = directSound->SetCooperativeLevel(window, DSSCL_PRIORITY);
				if (FAILED(resultValue))
				{
					throw std::runtime_error("Unable to set cooperative level");
				}
				...
				if (FAILED(resultValue))
				{
					throw std::runtime_error("Unable to create primary sound buffer");
				}
				...
				if (FAILED(resultValue))
				{
					throw std::runtime_error("Unable to set primary buffer format");
				}

				directSoundListener = primarySoundBuffer;
				if (!directSoundListener)
				{
					throw std::runtime_error("Unable to create primary 3D listener");
				}
