        ...
        , public Plugin::Visualizer
        {
          public:
            ...
            struct DrawCallValue
            {
                ...
                DrawCallValue(MaterialHandle material, VisualHandle plugin, ShaderHandle shader, std::function<void(Render::Device::Context *)> &&onDraw)
                    : material(material), plugin(plugin), shader(shader), onDraw(std::move(onDraw))
                {
                }
            };
            ...
            struct DrawCallSet
            {
                ...
                DrawCallSet(Engine::Shader *shader, DrawCallList::iterator begin, DrawCallList::iterator end)
                    : shader(shader), begin(begin), end(end)
                {
                }
            };
            ...
            struct Camera
            {
                ...
                Camera(Camera const &renderCall)
                    : name(renderCall.name), viewFrustum(renderCall.viewFrustum), viewMatrix(renderCall.viewMatrix), projectionMatrix(renderCall.projectionMatrix), nearClip(renderCall.nearClip), farClip(renderCall.farClip), cameraTarget(renderCall.cameraTarget), forceShader(renderCall.forceShader)
                {
                }
            };
          ...
          public:
            Visualizer(Context * context, Engine::Core * core)
                : ContextRegistration(context), core(core), renderDevice(core->getRenderDevice()), population(core->getPopulation()), resources(core->getFullResources()), directionalLightData(core->getRenderDevice()), pointLightData(core), spotLightData(core), workerPool(3)
            {
            ...
            void initializeSystem(void)
            {
                ...
                samplerList = {
                    textureSamplerState.get(),
                    mipMapSamplerState.get(),
                };
...
float4 mainPixelProgram(in Input input) : SV_TARGET0
{
	...
	return float4(inputBuffer[screenCoord], 1.0f);
}
)";

                deferredVertexProgram = resources->getProgram(Render::Program::Type::Vertex, "renderer:deferredVertexProgram:v10", "mainVertexProgram", vertexProgram);

                deferredPixelProgram = resources->getProgram(Render::Program::Type::Pixel, "renderer:deferredPixelProgram:v10", "mainPixelProgram", pixelProgram);
