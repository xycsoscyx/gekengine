    ...
    namespace Implementation
    {
        ...
        public:
                ...
                , videoDevice(videoDevice)
            {
                ...
                JSON::Instance visualNode = JSON::Load(getContext()->getRootFileName("data", "visuals", visualName).withExtension(".json"));

				std::string inputVertexData;
				std::vector<Video::InputElement> elementList;

                auto inputNode = visualNode.get("input");
                uint32_t inputIndexList[static_cast<uint8_t>(Video::InputElement::Semantic::Count)] = { 0 };
                for (const auto &elementNode : inputNode.getArray())
                {
                ...
                auto outputNode = visualNode.get("output");
				uint32_t outputIndexList[static_cast<uint8_t>(Video::InputElement::Semantic::Count)] = { 0 };
				for (const auto &elementNode : outputNode.getArray())
				{
                ...
                auto compiledProgram = resources->compileProgram(Video::PipelineType::Vertex, vertexFileName, vertexEntry, engineData);
				this->vertexProgram = videoDevice->createProgram(Video::PipelineType::Vertex, compiledProgram.data(), compiledProgram.size());
                ...
                if (visualNode.getObject().has_member("geometry"))
                {
                    ...
                    auto compiledProgram = resources->compileProgram(Video::PipelineType::Geometry, geometryFileName, geometryEntry);
                    this->geometryProgram = videoDevice->createProgram(Video::PipelineType::Geometry, compiledProgram.data(), compiledProgram.size());
