...
namespace Gek
{
    namespace Render
    {
        ...
        struct DisplayMode
        {
            ...
            inline DisplayMode(void) = default;
            inline DisplayMode(uint32_t width, uint32_t height, Format format)
                : width(width), height(height), format(format)
            {
                auto getAspectRatio = [](uint32_t width, uint32_t height) -> AspectRatio
                {
        ...
        struct ViewPort
        {
            ...
            ViewPort(Math::Float2 const &position, Math::Float2 const &size, float nearClip, float farClip)
                : position(position), size(size), nearClip(nearClip), farClip(farClip)
            {
            }
        };
            ...
            : virtual public Object
        {
            ...
            struct Information
            {
                ...
                Information(std::string_view name, Type type, std::string_view entryFunction, std::string_view shaderData, FileSystem::Path const &shaderPath)
                    : name(name), type(type), entryFunction(entryFunction), shaderData(shaderData), shaderPath(shaderPath)
                {
