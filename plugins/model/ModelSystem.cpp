    ...
    namespace Model
    {
        ...
        public:
            ...
            HRESULT loadShape(ModelData &data)
            {
                ...
                if (material)
                {
                    ...
                    else if (shape.CompareNoCase(L"sphere") == 0)
                    {
                        ...
                        resultValue = video->createBuffer(&data.vertexBuffer, sizeof(Vertex), geoSphere.getVertices().size(), Video::BufferType::Vertex, Video::BufferFlags::Static, geoSphere.getVertices().data());
                        if (SUCCEEDED(resultValue))
                        {
                            resultValue = video->createBuffer(&data.indexBuffer, Video::Format::Short, geoSphere.getIndices().size(), Video::BufferType::Index, Video::BufferFlags::Static, geoSphere.getIndices().data());
                        }
                    }
                    else
                    {
                ...
                if (SUCCEEDED(resultValue))
                {
                    ...
                    if (gekIdentifier == *(UINT32 *)"GEKX" && gekModelType == 0 && gekModelVersion == 2)
                    {
                        ...
                        if (SUCCEEDED(resultValue))
                        {
                            ...
                            resultValue = video->createBuffer(&data.vertexBuffer, sizeof(Vertex), vertexCount, Video::BufferType::Vertex, Video::BufferFlags::Static, rawFileData);
                        ...
                        if (SUCCEEDED(resultValue))
                        {
                            ...
                            resultValue = video->createBuffer(&data.indexBuffer, Video::Format::Short, indexCount, Video::BufferType::Index, Video::BufferFlags::Static, rawFileData);
            ...
            STDMETHODIMP initialize(IUnknown *initializerContext)
            {
                ...
                if (SUCCEEDED(resultValue))
                {
                    resultValue = video->createBuffer(&instanceBuffer, sizeof(InstanceData), 1024, Video::BufferType::Vertex, Video::BufferFlags::Dynamic);
                }
            ...
            STDMETHODIMP_(void) onEntityCreated(Engine::Population::Entity *entity)
            {
                ...
                if (entity->hasComponents<Model::Data, Engine::Components::Transform::Data>())
                {
                    auto &modelComponent = entity->getComponent<Model::Data>();
            ...
            STDMETHODIMP_(void) OnRenderScene(Engine::Population::Entity *cameraEntity, const Gek::Shape::Frustum *viewFrustum)
            {
                ...
                const auto &cameraTransform = cameraEntity->getComponent<Engine::Components::Transform::Data>();

                concurrency::concurrent_unordered_map<ModelData *, concurrency::concurrent_vector<InstanceData>> visibleList;
                concurrency::parallel_for_each(dataEntityList.begin(), dataEntityList.end(), [&](const std::pair<Engine::Population::Entity *, ModelData *> &dataEntity) -> void
                {
                    Engine::Population::Entity *entity = dataEntity.first;
                    ...
                    const auto &transformComponent = entity->getComponent<Engine::Components::Transform::Data>();
                    ...
                    if (viewFrustum->isVisible(orientedBox))
                    {
                        ...
                        if (entity->hasComponent<Engine::Components::Color::Data>())
                        {
                            color = entity->getComponent<Engine::Components::Color::Data>();
                        }
