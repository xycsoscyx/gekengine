...
namespace Gek
{
    ...
    public:
        ...
        VideoObjectPtr finishCommandList(void)
        {
            ...
            GEK_THROW_ERROR(!d3dCommandList, Video::Exception, "Unable to finish command list: %v", resultValue);
    ...
    public:
            ...
            , format(format)
        {
            ...
            GEK_THROW_ERROR(featureLevel != featureLevelList[0], Video::Exception, "Required Direect3D feature level not supported: %v", resultValue);
            GEK_THROW_ERROR(!dxSwapChain, Video::Exception, "Unable to create DirectX swap chain: %v", resultValue);
            GEK_THROW_ERROR(!d3dDevice, Video::Exception, "Unable to create Direct3D device %v", resultValue);
            GEK_THROW_ERROR(!d3dDeviceContext, Video::Exception, "Unable to create Direct3D device context: %v", resultValue);

            if (fullScreen && !isChildWindow)
            {
                resultValue = dxSwapChain->SetFullscreenState(true, nullptr);
                GEK_THROW_ERROR(FAILED(resultValue), Video::Exception, "Unable to set fullscreen state: %v", resultValue);
            }
        ...
        void setFullScreen(bool fullScreen)
        {
            if (!isChildWindow && this->fullScreen != fullScreen)
            {
                ...
                GEK_THROW_ERROR(FAILED(resultValue), Video::Exception, "Unable to change fullscreen state (%v): %v", (fullScreen ? "fullscreen" : "windowed"), resultValue);
            }
        }
        ...
        void setSize(UINT32 width, UINT32 height, Video::Format format)
        {
                ...
                DirectX::TextureFormatList[static_cast<UINT8>(format)] != modeDescription.Format)
            {
                ...
                GEK_THROW_ERROR(FAILED(resultValue), Video::Exception, "Unable to resize swap chain target: %v", resultValue);
            }
        }
        ...
        VideoTarget * const getBackBuffer(void)
        {
            if (!backBuffer)
            {
                ...
                GEK_THROW_ERROR(!d3dRenderTarget, Video::Exception, "Unable to get swap chain buffer: %v", resultValue);

                CComPtr<ID3D11RenderTargetView> d3dRenderTargetView;
                resultValue = d3dDevice->CreateRenderTargetView(d3dRenderTarget, nullptr, &d3dRenderTargetView);
                GEK_THROW_ERROR(!d3dRenderTargetView, Video::Exception, "Unable to create view object from swap chain: %v", resultValue);
        ...
        VideoContextPtr createDeferredContext(void)
        {
            ...
            GEK_THROW_ERROR(!d3dDeferredDeviceContext, Video::Exception, "Unable to create deferred context: %v", resultValue);
        ...
        VideoObjectPtr createEvent(void)
        {
            ...
            GEK_THROW_ERROR(!d3dQuery, Video::Exception, "Unable to create event object: %v", resultValue);
        ...
        VideoObjectPtr createRenderState(const Video::RenderState &renderState)
        {
            ...
            GEK_THROW_ERROR(!d3dStates, Video::Exception, "Unable to create render state: %v", resultValue);
        ...
        VideoObjectPtr createDepthState(const Video::DepthState &depthState)
        {
            ...
            GEK_THROW_ERROR(!d3dStates, Video::Exception, "Unable to create depth state: %v", resultValue);
        ...
        VideoObjectPtr createBlendState(const Video::UnifiedBlendState &blendState)
        {
            ...
            GEK_THROW_ERROR(!d3dStates, Video::Exception, "Unable to create unified blend state: %v", resultValue);
        ...
        VideoObjectPtr createBlendState(const Video::IndependentBlendState &blendState)
        {
            ...
            CComPtr<ID3D11BlendState> d3dStates;
            HRESULT resultValue = d3dDevice->CreateBlendState(&blendDescription, &d3dStates);
            GEK_THROW_ERROR(!d3dStates, Video::Exception, "Unable to create independent blend state: %v", resultValue);
        ...
        VideoObjectPtr createSamplerState(const Video::SamplerState &samplerState)
        {
            ...
            GEK_THROW_ERROR(!d3dStates, Video::Exception, "Unable to create sampler state: %v", resultValue);
        ...
        VideoBufferPtr createBuffer(Video::Format format, UINT32 stride, UINT32 count, Video::BufferType type, UINT32 flags, const void *data)
        {
            ...
            if (data == nullptr)
            {
                ...
                GEK_THROW_ERROR(!d3dBuffer, Video::Exception, "Unable to create dynamic buffer: %v", resultValue);
            }
            else
            {
                ...
                GEK_THROW_ERROR(!d3dBuffer, Video::Exception, "Unable to create static buffer: %v", resultValue);
            }
            ...
            if (flags & Video::BufferFlags::Resource)
            {
                ...
                GEK_THROW_ERROR(!d3dShaderResourceView, Video::Exception, "Unable to create buffer resource view: %v", resultValue);
            }
            ...
            if (flags & Video::BufferFlags::UnorderedAccess)
            {
                ...
                GEK_THROW_ERROR(!d3dUnorderedAccessView, Video::Exception, "Unable to create buffer unordered access view: %v", resultValue);
            }
        ...
        void mapBuffer(VideoBuffer *buffer, void **data, Video::Map mapping)
        {
            ...
            GEK_THROW_ERROR(FAILED(resultValue), Video::Exception, "Unable to map buffer: %v", resultValue);
        ...
        VideoObjectPtr compileComputeProgram(const wchar_t *fileName, const char *programScript, const char *entryFunction, const std::unordered_map<string, string> &defineList, ID3DInclude *includes)
        {
            ...
            GEK_THROW_ERROR(!d3dShaderBlob, Video::Exception, "Unable to compile shader data blob: %v", resultValue);

            CComPtr<ID3D11ComputeShader> d3dShader;
            resultValue = d3dDevice->CreateComputeShader(d3dShaderBlob->GetBufferPointer(), d3dShaderBlob->GetBufferSize(), nullptr, &d3dShader);
            GEK_THROW_ERROR(!d3dShader, Video::Exception, "Unable to create shader: %v", resultValue);
        ...
        VideoObjectPtr compileVertexProgram(const wchar_t *fileName, const char *programScript, const char *entryFunction, const std::vector<Video::InputElement> &elementLayout, const std::unordered_map<string, string> &defineList, ID3DInclude *includes)
        {
            ...
            if (!elementLayout.empty())
            {
                ...
                GEK_THROW_ERROR(!d3dInputLayout, Video::Exception, "Unable to create vertex input layout: %v", resultValue);
            }
        ...
        VideoTexturePtr createTexture(Video::Format format, UINT32 width, UINT32 height, UINT32 depth, UINT32 flags, UINT32 mipmaps)
        {
            ...
            if (flags & Video::TextureFlags::DepthTarget)
            {
                ...
                GEK_THROW_ERROR(depth > 1, Video::Exception, "Depth target can not have depth greater than 1: %v required", depth);
            ...
            if (depth == 1)
            {
                ...
                GEK_THROW_ERROR(!texture2D, Video::Exception, "Unable to create 2D texture: %v", resultValue);
            ...
            else
            {
                ...
                GEK_THROW_ERROR(!texture3D, Video::Exception, "Unable to create 3D texture: %v", resultValue);
            ...
            if (flags & Video::TextureFlags::Resource)
            {
                ...
                GEK_THROW_ERROR(!d3dShaderResourceView, Video::Exception, "Unable to create resource view: %v", resultValue);
            }
            ...
            if (flags & Video::TextureFlags::UnorderedAccess)
            {
                ...
                GEK_THROW_ERROR(!d3dUnorderedAccessView, Video::Exception, "Unable to create unordered access view: %v", resultValue);
            }
            ...
            if (flags & Video::TextureFlags::RenderTarget)
            {
                ...
                GEK_THROW_ERROR(!d3dRenderTargetView, Video::Exception, "Unable to create render target view: %v", resultValue);
            ...
            else if (flags & Video::TextureFlags::DepthTarget)
            {
                ...
                GEK_THROW_ERROR(!d3dDepthStencilView, Video::Exception, "Unable to create depth stencil view: %v", resultValue);
        ...
        VideoTexturePtr loadTexture(const wchar_t *fileName, UINT32 flags)
        {
            ...
            GEK_THROW_ERROR(!load, Video::Exception, "Invalid file type: %v", extension);
            HRESULT resultValue = load(fileData.data(), fileData.size(), &textureMetaData, scratchImage);
            GEK_THROW_ERROR(FAILED(resultValue), FileSystem::Exception, "Unable to load file: %v", resultValue);
        ...
        VideoTexturePtr loadCubeMap(const wchar_t *fileNameList[6], UINT32 flags)
        {
            ...
            GEK_THROW_ERROR(FAILED(resultValue), Video::Exception, "Unable to create cubemap from images: %v", resultValue);
