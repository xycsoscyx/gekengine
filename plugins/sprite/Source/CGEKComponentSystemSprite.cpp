...
STDMETHODIMP CGEKComponentSystemSprite::Initialize(void)
{
    ...
    if (SUCCEEDED(hRetVal))
    {
        ...
        hRetVal = m_pVideoSystem->CreateBuffer(sizeof(float4), 4, GEKVIDEO::BUFFER::VERTEX_BUFFER | GEKVIDEO::BUFFER::STATIC, &m_spVertexBuffer, aVertices);
        GEKRESULT(SUCCEEDED(hRetVal), L"Call to CreateBuffer failed: 0x%08X", hRetVal);
    }
...
STDMETHODIMP_(void) CGEKComponentSystemSprite::OnCullScene(void)
{
    ...
    m_pSceneManager->ListComponentsEntities({ L"transform", L"sprite" }, [&](const GEKENTITYID &nEntityID)->void
    {
        ...
        if (spMaterial)
        {
            ...
            m_aVisible[spMaterial].emplace_back(kPosition.GetFloat3(), kSize.GetFloat(), kColor.GetFloat4());
        }
    });
}

STDMETHODIMP_(void) CGEKComponentSystemSprite::OnDrawScene(UINT32 nVertexAttributes)
{
    ...
    for (auto kModel : m_aVisible)
    {
        for (UINT32 nPass = 0; nPass < kModel.second.size(); nPass += NUM_INSTANCES)
        {
            ...
            if (SUCCEEDED(m_spInstanceBuffer->Map((LPVOID *)&pInstances)))
            {
                ...
                if (m_pMaterialManager->EnableMaterial(kModel.first))
                {
                    m_pVideoSystem->GetImmediateContext()->DrawInstancedIndexedPrimitive(6, nNumInstances, 0, 0, 0);
                }
            }
        }
    }
