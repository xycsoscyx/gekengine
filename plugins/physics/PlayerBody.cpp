    ...
    namespace Newton
    {
        ...
        public:
                ...
                , falling(true)
            {
                ...
                NewtonBodySetMassProperties(newtonBody, physical.mass, bodyShape);
            ...
            {
                if (wcscmp(actionName, L"turn") == 0)
                {
                    headingAngle += (parameter.value * 0.01f);
                }
                else if (wcscmp(actionName, L"move_forward") == 0)
                {
                    moveForward = parameter.state;
                }
                else if (wcscmp(actionName, L"move_backward") == 0)
                {
                    moveBackward = parameter.state;
                }
                else if (wcscmp(actionName, L"strafe_left") == 0)
                {
                    strafeLeft = parameter.state;
                }
                else if (wcscmp(actionName, L"strafe_right") == 0)
                {
                    strafeRight = parameter.state;
                }
                else if (wcscmp(actionName, L"crouch") == 0)
                {
        ...
        */
        ...
        {
            if (wcscmp(actionName, L"crouch") == 0 && parameter.state)
            {
                return std::make_shared<CrouchingState>();
            }
            else if (wcscmp(actionName, L"move_forward") == 0 && parameter.state)
            {
                return std::make_shared<WalkingState>();
            }
            else if (wcscmp(actionName, L"move_backward") == 0 && parameter.state)
            {
                return std::make_shared<WalkingState>();
            }
            else if (wcscmp(actionName, L"strafe_left") == 0 && parameter.state)
            {
                return std::make_shared<WalkingState>();
            }
            else if (wcscmp(actionName, L"strafe_right") == 0 && parameter.state)
            {
                return std::make_shared<WalkingState>();
            }
            else if (wcscmp(actionName, L"jump") == 0 && parameter.state)
            {
        ...
        */
        ...
        {
            if (wcscmp(actionName, L"jump") == 0 && parameter.state)
            {
                return std::make_shared<JumpingState>();
            }
