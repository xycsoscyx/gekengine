...
IUnknown *CGEKComponentModel::GetModel(void) const
{
    return m_spModel;
}
...
STDMETHODIMP CGEKComponentSystemModel::Create(const CLibXMLNode &kNode, IGEKEntity *pEntity, IGEKComponent **ppComponent)
{
    HRESULT hRetVal = E_FAIL;
    if (kNode.HasAttribute(L"type") && kNode.GetAttribute(L"type").CompareNoCase(L"model") == 0)
    {
...
STDMETHODIMP CGEKComponentSystemModel::OnRender(void)
{
    ...
    {
        if (kPair.second->GetModel())
        {
            GetViewManager()->DrawModel(kPair.first, kPair.second->GetModel());
        }
    });
