        ...
        namespace Gizmo
        {
            ...
            struct Context
            {
                ...
                void ComputeCameraRay(vec_t &rayOrigin, vec_t &rayDir)
                {
                    ...
                    float mox = ((io.MousePos.x - mX) / mWidth) * 2.f - 1.f;
                    float moy = (1.f - ((io.MousePos.y - mY) / mHeight)) * 2.f - 1.f;
                ...
                void ComputeContext(const float *view, const float *projection, float *matrix, int mode)
                {
                    ...
                    mScreenFactor = 0.1f * GetUniform(mModel.v.position, mViewProjection);

                    ImVec2 centerSSpace = worldToPos(makeVect(0.f, 0.f), mMVP);
                ...
                void ComputeTripodAxisAndVisibility(int axisIndex, vec_t& dirPlaneX, vec_t& dirPlaneY, bool& belowAxisLimit, bool& belowPlaneLimit)
                {
                    ...
                    else
                    {
                        ...
                        dirPlaneNormalWorld.TransformVector(directionUnary[planNormal], mModel);
                ...
                float ComputeAngleOnPlan()
                {
                    ...
                    float acosAngle = Clamp(Dot(localPos, mRotationVectorSource), -0.9999f, 0.9999f);
                ...
                void DrawRotationGizmo(int type)
                {
                    ...
                    for (int axis = 0; axis < 3; axis++)
                    {
                        ...
                        float angleStart = atan2f(cameraToModelNormalized[(4 - axis) % 3], cameraToModelNormalized[(3 - axis) % 3]) + Math::Pi * 0.5f;
                    ...
                    drawList->AddCircle(worldToPos(mModel.v.position, mViewProjection), screenRotateSize * mHeight, colors[0], 64);
                    ...
                    if (mbUsing)
                    {
                        ...
                        char tmps[512];
                        ImFormatString(tmps, sizeof(tmps), rotationInfoMask[type - ROTATE_X], (mRotationAngle / Math::Pi)*180.f, mRotationAngle);
                ...
                void DrawHatchedAxis(const vec_t& axis)
                {
                    for (int j = 1; j < 10; j++)
                    {
                        ...
                        mDrawList->AddLine(baseSSpace2, worldDirSSpace2, 0x80000000, 6.f);
                    }
                }
                ...
                void DrawScaleGizmo(int type)
                {
                    ...
                    // draw screen cirle
                    drawList->AddCircleFilled(mScreenSquareCenter, 12.f, colors[0], 32);
                ...
                void DrawTranslationGizmo(int type)
                {
                    ...
                    // draw screen quad
                    drawList->AddRectFilled(mScreenSquareMin, mScreenSquareMax, colors[0], 2.f);
                ...
                void DrawBounds(float *bounds, matrix_t *matrix, float *snapValues)
                {
                    ImGuiIO& io = ImGui::GetIO();
                    ImDrawList* drawList = mDrawList;
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (!mbUsingBounds && mbEnable && overSmallAnchor && io.MouseDown[0])
                        {
                            ...
                            mBoundsLocalPivot.Set(0.f);
                            mBoundsLocalPivot[mBoundsAxis[0]] = aabb[oppositeIndex][indices[i % 2]];// bounds[mBoundsAxis[0]] * (((i + 1) & 2) ? 1.f : -1.f);
                ...
                int GetScaleType()
                {
                    ...
                    for (unsigned int i = 0; i < 3 && type == NONE; i++)
                    {
                        ...
                        const float len = IntersectRayPlane(mRayOrigin, mRayVector, BuildPlane(mModel.v.position, direction[planNormal]));
                        vec_t posOnPlan = mRayOrigin + mRayVector * len;

                        const float dx = dirPlaneX.Dot3((posOnPlan - mModel.v.position) * (1.f / mScreenFactor));
                        const float dy = dirPlaneY.Dot3((posOnPlan - mModel.v.position) * (1.f / mScreenFactor));
                ...
                int GetRotateType()
                {
                    ...
                    vec_t deltaScreen = { io.MousePos.x - mScreenSquareCenter.x, io.MousePos.y - mScreenSquareCenter.y, 0.f, 0.f };
                    ...
                    {
                        // pickup plane
                        vec_t pickupPlan = BuildPlane(mModel.v.position, planNormals[i]);
                ...
                void HandleTranslation(float *matrix, float *deltaMatrix, int& type, float *snap)
                {
                    ...
                    else
                    {
                        ...
                        if (io.MouseDown[0] && type != NONE)
                        {
                            ...
                            const vec_t movePlanNormal[] = { mModel.v.up, mModel.v.dir, mModel.v.right, mModel.v.dir, mModel.v.right, mModel.v.up, -mCameraDir };
                            // pickup plane
                            mTranslationPlan = BuildPlane(mModel.v.position, movePlanNormal[type - MOVE_X]);
                            const float len = IntersectRayPlane(mRayOrigin, mRayVector, mTranslationPlan);
                            mTranslationPlanOrigin = mRayOrigin + mRayVector * len;
                            mMatrixOrigin = mModel.v.position;

                            mRelativeOrigin = (mTranslationPlanOrigin - mModel.v.position) * (1.f / mScreenFactor);
                        }
                    }
                }
                ...
                void HandleScale(float *matrix, float *deltaMatrix, int& type, float *snap)
                {
                    ...
                    if (!mbUsing)
                    {
                        ...
                        if (io.MouseDown[0] && type != NONE)
                        {
                            mbUsing = true;
                            mCurrentOperation = type;
                            const vec_t movePlanNormal[] = { mModel.v.up, mModel.v.dir, mModel.v.right, mModel.v.dir, mModel.v.up, mModel.v.right, -mCameraDir };
                            // pickup plane

                            mTranslationPlan = BuildPlane(mModel.v.position, movePlanNormal[type - SCALE_X]);
                            const float len = IntersectRayPlane(mRayOrigin, mRayVector, mTranslationPlan);
                            mTranslationPlanOrigin = mRayOrigin + mRayVector * len;
                            mMatrixOrigin = mModel.v.position;
                    ...
                    if (mbUsing)
                    {
                        ...
                        if (mCurrentOperation >= SCALE_X && mCurrentOperation <= SCALE_Z)
                        {
                            ...
                            vec_t baseVector = mTranslationPlanOrigin - mModel.v.position;
                ...
                void HandleRotation(float *matrix, float *deltaMatrix, int& type, float *snap)
                {
                    ...
                    if (!mbUsing)
                    {
                        ...
                        if (io.MouseDown[0] && type != NONE)
                        {
                            mbUsing = true;
                            mCurrentOperation = type;
                            const vec_t rotatePlanNormal[] = { mModel.v.right, mModel.v.up, mModel.v.dir, -mCameraDir };
                            // pickup plane
                            if (applyRotationLocaly)
                            {
                                mTranslationPlan = BuildPlane(mModel.v.position, rotatePlanNormal[type - ROTATE_X]);
                            }
                            else
                            {
                                mTranslationPlan = BuildPlane(mModelSource.v.position, directionUnary[type - ROTATE_X]);
                            }
                    ...
                    if (mbUsing)
                    {
                        ...
                        rotationAxisLocalSpace.TransformVector(makeVect(mTranslationPlan.x, mTranslationPlan.y, mTranslationPlan.z, 0.f), mModelInverse);
                ...
                void HandleAndDrawBounds(float *bounds, matrix_t *matrix, float *snapValues)
                {
                    ImGuiIO& io = ImGui::GetIO();
                    ImDrawList* drawList = mDrawList;
                ...
                void DecomposeMatrixToComponents(const float *matrix, float *translation, float *rotation, float *scale)
                {
                    ...
                    rotation[0] = Math::RadiansToDegrees(atan2f(mat.m[1][2], mat.m[2][2]));
                    rotation[1] = Math::RadiansToDegrees(atan2f(-mat.m[0][2], sqrtf(mat.m[1][2] * mat.m[1][2] + mat.m[2][2] * mat.m[2][2])));
                    rotation[2] = Math::RadiansToDegrees(atan2f(mat.m[0][1], mat.m[0][0]));
                ...
                void Manipulate(const float *view, const float *projection, int operation, int mode, float *matrix, float *deltaMatrix, float *snap, float *localBounds, float *boundsSnap)
                {
                    ...
                    case Operation::Bounds:
                        HandleAndDrawBounds(localBounds, (matrix_t*)matrix, boundsSnap);
                        break;
                    };
                }
