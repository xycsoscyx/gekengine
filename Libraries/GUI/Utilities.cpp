...
#define GET_STYLE(OFFSET) (void *)((unsigned char *)&GImGui->Style + OFFSET)
        ...
        namespace Dock
        {
            struct DockContext
            {
                ...
                static ImRect getDockedRect(const ImRect& rect, Position dock_slot)
                {
                ...
                static ImRect getSlotRect(ImRect parent_rect, Position dock_slot)
                {
                ...
                static ImRect getSlotRectOnBorder(ImRect parent_rect, Position dock_slot)
                {
                    ImVec2 size = parent_rect.Max - parent_rect.Min;
                    ImVec2 center = parent_rect.Min + size * 0.5f;
                    switch (dock_slot)
                ...
                bool Positions(Dock& dock, Dock* dest_dock, const ImRect& rect, bool on_border)
                {
...
#	endif ////IMGUITABWINDOW_H_
                    ...
                    {
                        const Position iSlot = static_cast<Position>(i);
                        ImRect r =
...
#	endif ////IMGUITABWINDOW_H_
                ...
                void handleDrag(Dock& dock)
                {
                    ...
                    if (Positions(dock, nullptr, ImRect(m_workspace_pos, m_workspace_pos + m_workspace_size), true))
                    {
                        canvas->PopClipRect();
                        ImGui::End();
                        return;
                ...
                static void setDockPosSize(Dock& dest, Dock& dock, Position dock_slot, Dock& container, ImVec2 const &default_size)
                {
                    ...
                    if (dock_slot == Position::Bottom)
                    {
                        dest.size.y -= dock.size.y;
                        dock.pos.y += dest.size.y;
                    }
                    else if (dock_slot == Position::Right)
                    {
                        dest.size.x -= dock.size.x;
                        dock.pos.x += dest.size.x;
                    }
                    else if (dock_slot == Position::Left)
                    {
                        dest.size.x -= dock.size.x;
                        dest.pos.x += dock.size.x;
                    }
                    else if (dock_slot == Position::Top)
                    {
                        dest.size.y -= dock.size.y;
                ...
                void doDock(Dock& dock, Dock* dest, Position dock_slot, ImVec2 const &default_size = ImVec2(0.0f, 0.0f))
                {
                    ...
                    else if (dock_slot == Position::Tab)
                    {
                        Dock* tmp = dest;
                        while (tmp->next_tab)
                        {
                    ...
                    else if (dock_slot == Position::None)
                    {
                        dock.status = Status_Float;
                    }
                    else
                ...
                static Position getSlotFromLocationCode(char code)
                {
                    switch (code)
                    {
                    case '1':
                ...
                void tryDockToStoredLocation(Dock& dock)
                {
                    ...
                    doDock(dock, tmp ? tmp : prev, tmp && !tmp->children[0] ? Position::Tab : getSlotFromLocationCode(*c));
                }

                bool begin(char const *label, bool* opened, ImGuiWindowFlags extra_flags, const ImVec2& default_size)
                {
                    Position next_slot = m_next_dock_slot;
                    m_next_dock_slot = Position::Tab;
            ...
            void Begin(char const *label, ImVec2 const &workspace, bool showBorder, ImVec2 const &splitSize)
            {
            ...
            bool BeginTab(char const *label, bool *opened, ImGuiWindowFlags extra_flags, ImVec2 const &default_size)
            {
