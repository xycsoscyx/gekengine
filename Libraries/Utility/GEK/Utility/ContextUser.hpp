...
extern "C" __declspec(dllexport) void initializePlugin(                                                                                     \
    std::function<void(std::string_view, std::function<ContextUserPtr(Context *, void *, std::vector<std::type_index> &)>)> addClass,       \
...
namespace Gek
{
    ...
    public:
        ...
        static ContextUserPtr createFromPackedArguments(Context *context, const std::tuple<PARAMETERS...>& packedArguments, std::index_sequence<SIZE...> sequence, std::vector<std::type_index> &argumentTypes)
        {
            ...
            {
                LockedWrite{ std::cerr } << "Invalid number of arguments passed to constructor, received: " << sequence.size() << ", expected: " << argumentTypes.size();
                return nullptr;
            }
            ...
            {
                LockedWrite{ std::cerr } << "Parameter types passed to creator don't match constructor types";
                return nullptr;
            }
