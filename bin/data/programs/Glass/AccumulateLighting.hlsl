...
float3 mainPixelProgram(InputPixel inputPixel) : SV_TARGET0
{
    ...
    float3 glassColor = Resources::glassBuffer.SampleLevel(Global::linearClampSampler, (inputPixel.screen.xy * Shader::pixelSize), glassLevel);

    [branch]
    if (materialRoughness < 1.0)
    {
        float lerpLevel = frac(glassRoughness);
        glassColor = lerp(glassColor, Resources::glassBuffer.SampleLevel(Global::linearClampSampler, (inputPixel.screen.xy * Shader::pixelSize), (glassLevel + 1)), lerpLevel);
    }

    return surfaceIrradiance + glassColor;
}
