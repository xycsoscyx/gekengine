...
OutputPixel mainPixelProgram(InputPixel inputPixel)
{
    ...
    float delta = saturate((sceneDepth - depth) * Camera::maximumDistance);

    float4 color = (Resources::albedo.Sample(Global::linearWrapSampler, inputPixel.texCoord) * inputPixel.color * float4((1.0).xxx, delta));
