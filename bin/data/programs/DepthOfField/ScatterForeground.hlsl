...
float3 getChromaticAberation(float2 texCoord, float blur)
{
    ...
    return float3(
        Resources::foregroundBuffer.SampleLevel(Global::pointSampler, texCoord + float2(0.0, 1.0) * offset, 0).r,
        Resources::foregroundBuffer.SampleLevel(Global::pointSampler, texCoord + float2(-0.866, -0.5) * offset, 0).g,
        Resources::foregroundBuffer.SampleLevel(Global::pointSampler, texCoord + float2(0.866, -0.5) * offset, 0).b);
}
...
float3 getColor(float2 texCoord, float blur)
{
    ...
    {
        return Resources::foregroundBuffer.SampleLevel(Global::pointSampler, texCoord + Shader::pixelSize * blur, 0);
    }
}
