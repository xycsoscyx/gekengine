...
HRESULT GEKOptimizeMesh(const float3 *pInputVertices, UINT32 nNumVertices, const UINT16 *pInputIndices, UINT32 nNumIndices,
                        std::vector<float3> &aOutputVertices, std::vector<UINT16> &aOutputIndices, float nFaceEpsilon)
{
    REQUIRE_RETURN(pInputVertices, E_INVALIDARG);
    REQUIRE_RETURN(nNumVertices > 0, E_INVALIDARG);
        ...
        if (hWindow)
        {
            ...
            if (spDirect3D)
            {
                ...
                if (spDevice)
                {
                    ...
                    if (spMesh)
                    {
                        float3 *pVertices = nullptr;
                        spMesh->LockVertexBuffer(D3DLOCK_DISCARD, (void **)&pVertices);
                        memcpy(pVertices, pInputVertices, (sizeof(float3) * nNumVertices));
                        spMesh->UnlockVertexBuffer();
                        ...
                        if (SUCCEEDED(hRetVal))
                        {
                            ...
                            if (SUCCEEDED(hRetVal))
                            {
                                spMesh->LockVertexBuffer(D3DLOCK_READONLY, (void **)&pVertices);
                                aOutputVertices.assign(pVertices, &pVertices[spMesh->GetNumVertices()]);
                                spMesh->UnlockVertexBuffer();

                                spMesh->LockIndexBuffer(D3DLOCK_READONLY, (void **)&pIndices);
                                aOutputIndices.assign(pIndices, &pIndices[spMesh->GetNumFaces() * 3]);
                                spMesh->UnlockIndexBuffer();
                            }
                        }
                    }
